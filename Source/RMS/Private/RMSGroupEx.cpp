                                                          ...
                                                          const UCharacterMovementComponent& MoveComponent)
{
	...
	if (Duration > SMALL_NUMBER && MovementTickTime > SMALL_NUMBER)
	{
		...
		if (CurrData.RotationSetting.IsWarpRotation())
		{
			...
			const float RotationFraction = FMath::Clamp(MoveFraction * CurrData.RotationSetting.WarpMultiplier, 0.0f,
			                                            1.0f);

			URMSLibrary::ExtractRotation(RotationDt, Character, LastData.RotationSetting.TargetRotation, TargetRotation,
			                             RotationFraction,
...
//
// //**********************************V2****************************************
// FRootMotionSource_PathMoveToForce_V2::FRootMotionSource_PathMoveToForce_V2()
// {
// }
//
// bool FRootMotionSource_PathMoveToForce_V2::UpdateStateFrom(const FRootMotionSource* SourceToTakeStateFrom,
//                                                            bool bMarkForSimulatedCatchup)
// {
// 	if (!FRootMotionSource::UpdateStateFrom(SourceToTakeStateFrom, bMarkForSimulatedCatchup))
// 	{
...
// }
//
// void FRootMotionSource_PathMoveToForce_V2::PrepareRootMotion(float SimulationTime, float MovementTickTime,
//                                                              const ACharacter& Character,
//                                                              const UCharacterMovementComponent& MoveComponent)
// {
// 	RootMotionParams.Clear();
...
// }
//
// bool FRootMotionSource_PathMoveToForce_V2::NetSerialize(FArchive& Ar, UPackageMap* Map, bool& bOutSuccess)
// {
// 	if (!FRootMotionSource::NetSerialize(Ar, Map, bOutSuccess))
// 	{
// 		return false;
...
// }
//
// FRootMotionSource* FRootMotionSource_PathMoveToForce_V2::Clone() const
// {
// 	FRootMotionSource_PathMoveToForce_V2* CopyPtr = new FRootMotionSource_PathMoveToForce_V2(*this);
// 	return CopyPtr;
// }
//
// bool FRootMotionSource_PathMoveToForce_V2::Matches(const FRootMotionSource* Other) const
// {
// 	if (!FRootMotionSource::Matches(Other))
// 	{
// 		return false;
// 	}
//
// 	const FRootMotionSource_PathMoveToForce_V2* OtherCast = static_cast<const FRootMotionSource_PathMoveToForce_V2*>(
// 		Other);
//
// 	return Path == OtherCast->Path && StartRotation == OtherCast->StartRotation;
// }
//
// bool FRootMotionSource_PathMoveToForce_V2::MatchesAndHasSameState(const FRootMotionSource* Other) const
// {
// 	if (!FRootMotionSource::MatchesAndHasSameState(Other))
// 	{
// 		return false;
...
// }
//
// UScriptStruct* FRootMotionSource_PathMoveToForce_V2::GetScriptStruct() const
// {
// 	return FRootMotionSource_PathMoveToForce_V2::StaticStruct();
// }
//
// FString FRootMotionSource_PathMoveToForce_V2::ToSimpleString() const
// {
// 	return FString::Printf(
// 		TEXT("[ID:%u]FRootMotionSource_PathMoveToForce_V2 %s"), LocalID, *InstanceName.GetPlainNameString());
// }
//
// void FRootMotionSource_PathMoveToForce_V2::AddReferencedObjects(FReferenceCollector& Collector)
// {
// 	FRootMotionSource::AddReferencedObjects(Collector);
// }
...
bool FRootMotionSource_JumpForce_WithPoints::UpdateStateFrom(const FRootMotionSource* SourceToTakeStateFrom,
                                                             bool bMarkForSimulatedCatchup)
{
	if (!FRootMotionSource::UpdateStateFrom(SourceToTakeStateFrom, bMarkForSimulatedCatchup))
	{
...
void FRootMotionSource_JumpForce_WithPoints::PrepareRootMotion(float SimulationTime, float MovementTickTime,
                                                               const ACharacter& Character,
                                                               const UCharacterMovementComponent& MoveComponent)
{
	if (!bIsInit)
	...
	if (Duration > SMALL_NUMBER && MovementTickTime > SMALL_NUMBER && SimulationTime > SMALL_NUMBER)
	{
		...
		if (RotationSetting.Mode == ERMSRotationMode::FaceToTarget)
		{
			...
			{
				RotFraction = FMath::Clamp(
					RotationSetting.WarpMultiplier * URMSLibrary::EvaluateFloatCurveAtFraction(
						*RotationSetting.Curve, RotFraction), 0.f, 1.f);
			}
		...
		if (RMS::CVarRMS_Debug.GetValueOnGameThread() > 0)
		{
			...
			DrawDebugCapsule(Character.GetWorld(), HalfwayLocation + LocDiff, Character.GetSimpleCollisionHalfHeight(),
			                 Character.GetSimpleCollisionRadius(), FQuat::Identity, FColor::White, true, DebugLifetime);

			// Destination point
			const FVector DestinationLocation = CurrentLocation + (GetRelativeLocation(1.0f) - CurrentRelativeLocation);
			DrawDebugCapsule(Character.GetWorld(), DestinationLocation + LocDiff,
			                 Character.GetSimpleCollisionHalfHeight(), Character.GetSimpleCollisionRadius(),
			                 FQuat::Identity, FColor::White, true, DebugLifetime);

			UE_LOG(LogRootMotion, VeryVerbose,
			       TEXT("RootMotionJumpForce %s %s preparing from %f to %f from (%s) to (%s) resulting force %s"),
...

			{
				FString AdjustedDebugString = FString::Printf(
					TEXT(
						"    FRootMotionSource_JumpForce::Prep Force(%s) SimTime(%.3f) MoveTime(%.3f) StartP(%.3f) EndP(%.3f)"),
					*Force.ToCompactString(), SimulationTime, MovementTickTime, CurrentMoveFraction,
					TargetMoveFraction);
...
void FRootMotionSource_JumpForce_WithPoints::InitPath(const ACharacter& Character)
{
	...
	for (float i = 0; i <= Duration; i += Duration / 64.0)
	{
		...
		{
			FVector CurrPoint = HalfWayLocation + ((CurrDist - Dist1toMid) / DistMidto2) * (TargetLocation -
				HalfWayLocation);
			X = (CurrPoint - StartLocation).Size2D();
			if (bDebug)
			{
		...
		{
			FVector CurrPoint = AvgMidPoint + ((CurrDistAvg - Dist1toAvgMid) / DistAvgMidto2) * (TargetLocation -
				AvgMidPoint);
			AvgX = (CurrPoint - StartLocation).Size2D();
			if (bDebug)
			{
	...
	if (RotationSetting.IsWarpRotation())
	{
		...
		if (Duration > SMALL_NUMBER && MovementTickTime > SMALL_NUMBER)
		{
			...
			const FRotator TargetRotation = RotationSetting.Mode == ERMSRotationMode::Custom
				                                ...
				                                : (TargetLocation - StartLocation).Rotation();
			const float RotationFraction = FMath::Clamp(MoveFraction * RotationSetting.WarpMultiplier, 0.f, 1.f);
...
bool FRootMotionSource_MoveToForce_WithRotation::Matches(const FRootMotionSource* Other) const
{
	...
	const FRootMotionSource_MoveToForce_WithRotation* OtherCast = static_cast<const
		FRootMotionSource_MoveToForce_WithRotation*>(Other);
...
bool FRootMotionSource_MoveToForce_WithRotation::UpdateStateFrom(const FRootMotionSource* SourceToTakeStateFrom,
                                                                 bool bMarkForSimulatedCatchup)
{
	if (!FRootMotionSource::UpdateStateFrom(SourceToTakeStateFrom, bMarkForSimulatedCatchup))
	{
...
{
	FRootMotionSource_MoveToDynamicForce_WithRotation* CopyPtr = new
		FRootMotionSource_MoveToDynamicForce_WithRotation(*this);
	return CopyPtr;
}
...
bool FRootMotionSource_MoveToDynamicForce_WithRotation::Matches(const FRootMotionSource* Other) const
{
	...
	const FRootMotionSource_MoveToDynamicForce_WithRotation* OtherCast = static_cast<const
		FRootMotionSource_MoveToDynamicForce_WithRotation*>(Other);
                                                            ...
                                                            float InCurrentTime, float DeltaSeconds)
{
	...
	if (RotationSetting.IsWarpRotation())
	{
		...
		if (RotationSetting.Curve)
		{
			TimeRemaining = EndTime - FMath::Clamp(RotationSetting.Curve->GetFloatValue(Fraction), 0.f, 1.f) * EndTime;
		}
                                                  ...
                                                  float DeltaSeconds)
{
	...
	const float PercentThisStep = FMath::Clamp(
		DeltaSeconds / TimeRemaining * FMath::Max(0.01f, RotationSetting.WarpMultiplier), 0.f, 1.f);
